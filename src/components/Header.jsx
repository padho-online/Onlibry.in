      ...
      <div className="container mx-auto px-4 py-3 max-w-7xl">
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-green-600 hidden sm:inline">Onlibry</span>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <NavLink to="/mock-tests" icon={FileQuestion} label="Mock Tests" />
            <NavLink to="/quizzes" icon={HelpCircle} label="Quiz" />
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-2">
              {user ? (
                ...
                <button onClick={handleLogin} className="p-2 rounded-lg bg-green-600 hover:bg-green-700 transition">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-4 pt-4 border-t border-gray-200">
            <nav className="flex flex-col gap-1">
              ...
              <NavLink to="/files" icon={FileText} label="Files" onClick={() => setMobileMenuOpen(false)} />
              <NavLink to="/folders" icon={Folder} label="Folders" onClick={() => setMobileMenuOpen(false)} />
              <NavLink to="/mock-tests" icon={FileQuestion} label="Mock Tests" onClick={() => setMobileMenuOpen(false)} />
              <NavLink to="/quizzes" icon={HelpCircle} label="Quiz" onClick={() => setMobileMenuOpen(false)} />
              {user && <NavLink to="/saved-files" icon={Bookmark} label="Saved" onClick={() => setMobileMenuOpen(false)} />}
              <NavLink to="/pricing" icon={CreditCard} label="Pricing" onClick={() => setMobileMenuOpen(false)} />
              
              <div className="pt-3 mt-2 border-t border-gray-200">
                {user ? (
                  <>
                    <span className="block text-sm text-gray-600 py-2 px-3">Hi, {user.displayName || user.email}</span>
                    <button
