// src/pages/FoldersPage.jsx - Mobile optimized with subpage navigation
...
function FoldersPage() {
  ...
  const openFolder = (folderName, folderData) => {
    if (folderData.isLeaf) {
      // For files - navigate to FilesPage with subpage search using full path
      if (folderData.link) {
        window.open(folderData.link, '_self');
      } else {
        // Build search query from full path + file name
        const searchTerms = [...currentPath, folderName].join(' ');
        const encodedQuery = encodeURIComponent(searchTerms).replace(/%20/g, '+');
        navigate(`/files/search/subpage/${encodedQuery}`);
      }
      return;
    }
  ...
  const handleFileClick = (fileName, fileData, currentPathArr) => {
    if (fileData.link) {
      window.open(fileData.link, '_self');
    } else {
      const searchTerms = [...currentPathArr, fileName].join(' ');
      const encodedQuery = encodeURIComponent(searchTerms).replace(/%20/g, '+');
      navigate(`/files/search/subpage/${encodedQuery}`);
    }
  };
      ...
      return (
        <div key={pathKey} className="mb-1">
          <div
            ...
            onClick={() => {
              if (node.isLeaf) {
                handleFileClick(name, node, currentPathArr);
              } else {
                openFolder(name, node);
              }
            }}
          >
            {hasChildren && (
  ...
  return (
    <div className="py-3 md:py-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <button 
          onClick={() => setViewMode(viewMode === 'filemanager' ? 'tree' : 'filemanager')} 
      ...
      <form onSubmit={handleSearch} className="flex gap-2 mb-4">
        ...
        {searchQuery && (
          <button type="button" onClick={handleClearSearch} className="px-3 py-2 bg-gray-200 rounded-lg">
            <X size={14} />
          </button>
        )}
      </form>
      ...
      {viewMode === 'filemanager' ? (
        <div>
          ...
          {currentPath.length > 0 && (
            <button onClick={goBack} className="mb-3 px-3 py-1.5 bg-gray-200 rounded-lg text-xs flex items-center gap-1">
              ← Back
            </button>
          )}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
            {currentItems.map(item => (
              <div 
                ...
                className="bg-white rounded-xl shadow-md p-3 border border-gray-200 cursor-pointer text-center"
              >
                {item.type === 'folder' ? (
                  <Folder size={24} className="text-orange-500 mx-auto mb-2" />
                ) : (
                  <File size={24} className="text-green-600 mx-auto mb-2" />
                )}
