// src/pages/QuizzesPage.jsx - with Search Logs to Sheet + Debounce + Page Path + URL Params
      ...
      {showFilters && (
        <div className="mb-4 p-3 bg-gray-50 rounded-lg">
          <div className="flex flex-wrap gap-2 mb-2">
            <button onClick={() => handleCategoryChange('all')} className={`px-3 py-1 rounded-full text-xs ${selectedCategory === 'all' ? 'bg-purple-600 text-white' : 'bg-gray-200'}`}>All</button>
            {categories.map(cat => <button key={cat} onClick={() => handleCategoryChange(cat)} className={`px-3 py-1 rounded-full text-xs ${selectedCategory === cat ? 'bg-purple-600 text-white' : 'bg-gray-200'}`}>{cat}</button>)}
          </div>
          {subCategories.length > 0 && (
            <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200">
              <button onClick={() => handleSubCategoryChange('all')} className={`px-2 py-0.5 rounded-full text-[10px] ${selectedSubCategory === 'all' ? 'bg-purple-600 text-white' : 'bg-gray-200'}`}>All</button>
              {subCategories.map(sub => <button key={sub} onClick={() => handleSubCategoryChange(sub)} className={`px-2 py-0.5 rounded-full text-[10px] ${selectedSubCategory === sub ? 'bg-purple-600 text-white' : 'bg-gray-200'}`}>{sub}</button>)}
            </div>
          )}
        </div>
      )}
